...
import {NFTType, SelectedCollectInformationType, selectedEventType} from "@/util/types/types";
import {getImage} from "@/util/externals/storage/web3";
import {selectedCollectInformation, selectedItems} from "@/util/statics/data";
import {SelectCollectInformationType} from "@/util/enums/enum";
import dayjs from "dayjs";

export default function Detail({params} : {params: { enterCode: string }}) {
  const account = useAccount()
  const router = useRouter()
  const crateNFT = useCreateNFT()
  const [nftInfo, setNFT] = useState<NFTType>()
  const [imageUrl, setImageUrl] = useState('')
  const [eventType, setEventType] = useState<selectedEventType>()
  const [collectInformationType, setCollectInformationType] = useState<SelectedCollectInformationType>()

  useEffect(() => {
    if (!account.isLoggedIn()) {
      router.replace('/')
    }
    const nft = crateNFT.NFTList.find(item => item.nft_collection_id === params.enterCode)
    if (nft) {
      setNFT(nft)
      const eventType = selectedItems.find(item => item.type === nft.event_type)
      if (eventType) {
        setEventType(eventType)
      }
      const collectInformation = selectedCollectInformation.find(item => item.type === nft.collect_information_type)
      if(collectInformation) {
        setCollectInformationType(collectInformation)
      }
